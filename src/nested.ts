...
export function findQuestion(
    ...
    const qst = questions.find((question: Question) => question.id === id);
    return qst || null;
}
...
export function toCSV(questions: Question[]): string {
    ...
    const rows = questions.map((question) => {
        ...
        return `${question.id},${question.name},${count},${question.points},${question.published}`;
    });
...
export function editOption(
    ...
    return questions.map((question) => {
        if (question.id === targetId) {
            const upd = [...question.options];
            if (targetOptionIndex === -1) {
                upd.push(newOption);
            } else {
                upd[targetOptionIndex] = newOption;
            }

            return { ...question, options: upd };
        }
        return question;
    });
}
